...
func _update_best_time() -> void:
	var new_best_time := floori(current_map_time)
	if (
			not map_save_data.has(KEY_BEST_TIME)
			or new_best_time <= map_save_data[KEY_BEST_TIME]
	):
		map_save_data[KEY_BEST_TIME] = new_best_time
		HUD.show_map_best_time(new_best_time)
